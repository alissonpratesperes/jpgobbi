...
hamburger.addEventListener('click', () => {
    ...
    mobile_menu.classList.toggle('active'); 
        function checkScroll() {
            if (hamburger.classList.contains('active')) { document.getElementsByTagName('html')[0].classList.add('dontScroll'); }
        ...
        let currentActive = 0;
            window.addEventListener('scroll', () => {
                const current = sections.length - [...sections].reverse().findIndex((section) => window.scrollY >= section.offsetTop - sectionMargin) - 1;
                    if(current !== currentActive) { removeAllActive(); currentActive = current; makeActive(current); }
                });
}, false);
...
function smoothScroll(target, duration) {
    ...
    var contact_link = document.querySelector('section#header div.header div.nav_bar div.nav_list ul li.nav_link a#contact_link');
        ...
        scroll_button.addEventListener('click', function() { smoothScroll('section#services', 700); });
        emblem_link.addEventListener('click', function() { smoothScroll('section#hero', 700); });
        hero_link.addEventListener('click', function() { smoothScroll('section#hero', 700); });
        services_link.addEventListener('click', function() { smoothScroll('section#services', 700); });
        projects_link.addEventListener('click', function() { smoothScroll('section#projects', 700); });
        contact_link.addEventListener('click', function() { smoothScroll('section#contact', 700); });
...
var smartNavBarCall = document.querySelector('section#header');
    window.addEventListener('scroll', function(e) {
        last_scroll_position = window.scrollY;
            if(new_scroll_position < last_scroll_position && last_scroll_position > 80) { smartNavBarCall.style.top = '-150px'; }
                else if ( new_scroll_position > last_scroll_position ) { smartNavBarCall.style.top = '0px'; }
